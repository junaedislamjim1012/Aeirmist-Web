...
export const AeirmistDashboard: React.FC<AeirmistDashboardProps> = ({ onUserClick, onMessageClick }) => {
  ...
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('aeirmist_recent_searches_v2');
  ...
  const addRecentSearch = (term: string) => {
    ...
    setRecentSearches(prev => {
      ...
      try {
        localStorage.setItem('aeirmist_recent_searches_v2', JSON.stringify(updated));
      } catch {}
      return updated;
    });
  };
          ...
          <div className={`${isGlobalBgActive ? 'bg-[#030206]/30' : 'bg-[#030206]/95'} backdrop-blur-3xl px-6 py-4 border-b border-white/5 flex gap-4 items-center justify-between shrink-0 relative z-40`}>
            <div className="relative w-full max-w-md">
              <div className="relative w-full">
                {isGlobalBgActive ? (
                  <div className="absolute left-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg bg-white/10 backdrop-blur-md border border-white/10 flex items-center justify-center shrink-0 z-10">
                    <Search className={`w-3 h-3 transition-colors duration-300 ${isSearchFocused ? 'text-aeirmist-cyan' : 'text-white/30'}`} />
                  </div>
                ) : (
                  <Search className={`w-3.5 h-3.5 absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${isSearchFocused ? 'text-aeirmist-cyan' : 'text-white/30'}`} />
                )}
                <input 
                  type="text" 
                  value={searchQuery}
                  ...
                  className={`w-full bg-[#0d0b12] border border-white/10 rounded-xl ${isGlobalBgActive ? 'pl-14' : 'pl-11'} pr-10 py-3 text-[11px] text-white placeholder-white/20 focus:outline-none focus:border-aeirmist-cyan/40 focus:ring-1 focus:ring-aeirmist-cyan/10 transition-all font-mono`}
                />
                {searchQuery && (
                  <button 
                    onClick={() => {
                        ...
                        <div className="space-y-2.5">
                          ...
                          <div className="flex flex-col gap-1 max-h-48 overflow-y-auto no-scrollbar">
                            {recentSearches.map((term, idx) => (
                              ...
                              >
                                <span className="truncate whitespace-nowrap min-w-0 flex-1">{term}</span>
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                ...
                {activeTab === 'for-you' ? (
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xs font-black uppercase tracking-widest text-[#00f2ff]">
                      {activeTab} ({activeTabProfiles.length})
                    </h3>
      ...
      <div className="flex md:hidden flex-col w-full h-full relative" id="mobile-connections-layout">
        ...
        <div className={`sticky top-0 ${isGlobalBgActive ? 'bg-[#030206]/35' : 'bg-[#030206]/98'} backdrop-blur-3xl z-30 pt-4 pb-3 border-b border-white/5 px-4 flex flex-col shrink-0 gap-3.5`}>
          ...
          <div className="flex items-center justify-between pb-1">
            ...
            <span className="text-[8px] font-mono tracking-widest text-[#00f2ff] uppercase border border-aeirmist-cyan/20 px-2 py-0.5 rounded-md bg-aeirmist-cyan/5">
              user space
            </span>
          </div>
          ...
          <div className="relative w-full z-40">
            <div className="relative w-full">
              ...
              <input 
                ...
                className={`w-full bg-[#110f17] border border-white/10 rounded-xl ${isGlobalBgActive ? 'pl-14' : 'pl-11'} pr-10 py-3 text-[11px] text-white placeholder-white/20 focus:outline-none focus:border-aeirmist-cyan/40 focus:ring-1 focus:ring-aeirmist-cyan/10 transition-all font-mono`}
              />
              {searchQuery && (
                <button 
