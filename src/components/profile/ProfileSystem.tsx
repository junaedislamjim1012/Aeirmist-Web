...
const ProfileSystem = ({ targetProfile, onMessageClick, onEditProfile, onUserClick, onCreate }: { targetProfile?: any, onMessageClick?: (user: any) => void, onEditProfile?: () => void, onUserClick?: (user: any) => void, onCreate?: () => void }) => {
  ...
  React.useEffect(() => {
    if (!db || !targetProfile?.id || targetProfile.id === profile?.id) {
    ...
    return () => unsub();
  }, [db, targetProfile?.id, profile?.id]);
          ...
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-10 items-center lg:items-start text-center lg:text-left">
            ...
            <div className="flex-1 flex flex-col pt-2 items-center lg:items-start w-full">
              ...
              <div className="flex gap-3 sm:gap-4 mb-5 w-full justify-center lg:justify-start overflow-x-auto pb-1.5">
                <GlassStat value={(posts || []).filter(p => p && !p.isArchived).length} label="POSTS" />
                <GlassStat 
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-6 justify-between px-4">
              ...
              <div className="flex flex-1 justify-around max-w-sm pl-2">
                <MobileStatItem value={(posts || []).filter(p => p && !p.isArchived).length} label="Posts" />
                <MobileStatItem 
                  value={Array.isArray(displayUser?.social?.followers) ? displayUser.social.followers.length : Math.max(0, displayUser?.followersCount || 0)} 
                  label="Followers" 
                  onClick={() => !isLocked && handleShowFollowList('followers')}
            ...
            <div className="space-y-1.5 px-4">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div onClick={() => setIsRankDetailModalOpen(true)} className="cursor-pointer active:scale-95 transition-transform">
                  <AeirmistRankBadge score={displayUser?.aeirmistLevel || 0} size="xs" />
                </div>
              </div>
        ...
        {isEditingBio && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
               ...
               <h2 className="text-xl sm:text-2xl font-black uppercase tracking-[0.2em] mb-6 sm:mb-10 text-center shrink-0">Digital Configuration</h2>
                ...
                {mutualConnections.length > 0 ? (
                  mutualConnections.map(m => (
                    ...
                    >
                       ...
                       <span className="text-[9px] text-aeirmist-cyan font-mono font-bold uppercase tracking-wider px-2 py-1 bg-aeirmist-cyan/10 rounded-lg">Verified</span>
                    </div>
                  ))
                ) : (
...
const GlassStat = ({ value, label, onClick, disabled, isProminent }: { value: number, label: string, onClick?: () => void, disabled?: boolean, isProminent?: boolean }) => (
  <motion.div 
    whileHover={!disabled ? { y: -4, backgroundColor: isProminent ? 'rgba(0,242,255,0.08)' : 'rgba(255,255,255,0.06)' } : {}}
    ...
    } ${onClick && !disabled ? 'cursor-pointer active:scale-95' : 'cursor-default'} ${disabled ? 'opacity-40 grayscale pointer-events-none' : ''}`}
  >
...
const MobileStatItem = ({ value, label, onClick, disabled, isProminent }: { value: number, label: string, onClick?: () => void, disabled?: boolean, isProminent?: boolean }) => (
  <div 
    onClick={!disabled ? onClick : undefined}
            ...
            return (
              <div className="relative">
                ...
                <PostMenu 
                  ...
                  onReport={() => submitReport({ targetType: 'post', targetId: livePost.id, reason: 'inappropriate_content' })}
                  onEdit={() => {
                    setIsMenuOpen(false);
                    setIsEditing(true);
                  }}
