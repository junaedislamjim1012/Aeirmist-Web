...
export const PremiumPostCard = React.memo<PostCardProps>(({ post, onUserClick, onNavigate }) => {
  ...
  return (
    <motion.div 
      ...
      className="glass-panel rounded-2xl sm:rounded-[2.2rem] border-white/5 bg-black/40 overflow-hidden mb-4 sm:mb-6 group shadow-2xl hover:border-white/10 transition-all duration-500 text-left relative"
    >
      ...
      <div className="p-3 sm:p-5 flex items-center justify-between">
        ...
        <PostMenu 
          ...
          onReport={() => submitReport({ targetType: 'post', targetId: post.id, reason: 'inappropriate_content' })}
          onPin={handlePin}
          onEdit={handleEdit}
        ...
        <div className={shouldHideSensitive ? 'blur-2xl pointer-events-none select-none opacity-20' : ''}>
          {isEditing ? (
            ...
            isShortTextOnly ? (
              ...
              <div className="px-5 pb-3.5 sm:px-6 sm:pb-4 text-left">
          ...
          {post.music && (
            <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/5 w-fit ml-4 sm:ml-6 mb-4 select-none">
          ...
          {hasMedia && (
            <div
              className="w-full border-y border-white/5 bg-black/20"
              onClickCapture={(e) => {
            ...
            >
              <Collage items={collageItems} fitMode={(post as any).fitMode || 'cover'} />
            </div>
          )}
        </div>
      </div>
